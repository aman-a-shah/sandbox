...
import { closeRecipeBook, createShopState, drawWorkstation, getRecipeBookPageCount, getSelectedRecipe, isPlayerNearWorkstation, openRecipeBook, renderRecipeBook, setRecipeBookRecipes, updateWorkstationPrompt } from "./features/shop";
...
let craftToastTimeoutId: number | null = null;
...
function showCraftToast(message: string): void {
  ...
  if (craftToastTimeoutId !== null) {
    window.clearTimeout(craftToastTimeoutId);
  }

  craftToastTimeoutId = window.setTimeout(() => {
    craftToastEl.classList.add("is-hidden");
    craftToastTimeoutId = null;
  }, 2200);
}
